...
WORDPRESS_DB_NAME=$(grep -Pom 1 "(?<=^WordpressDBName = ).*$" /root/NeXt-Server-Bullseye/wordpress_login_data.txt)
WordpressDBUser=$(grep -Pom 1 "(?<=^WordpressDBUser = ).*$" /root/NeXt-Server-Bullseye/wordpress_login_data.txt)
WordpressScriptPath=$(grep -Pom 1 "(?<=^WordpressScriptPath = ).*$" /root/NeXt-Server-Bullseye/wordpress_login_data.txt)
...
rm /root/NeXt-Server-Bullseye/wordpress_login_data.txt
...
sed_replace_word "$WORDPRESS_PATH_NAME" "" "/root/NeXt-Server-Bullseye/configs/blocked_paths.conf"
sed_replace_word "WORDPRESS_PATH_NAME=\".*"\" "WORDPRESS_PATH_NAME=\"0"\" "/root/NeXt-Server-Bullseye/configs/userconfig.cfg"
sed_replace_word "WORDPRESS_IS_INSTALLED=\"1"\" "WORDPRESS_IS_INSTALLED=\"0"\" "/root/NeXt-Server-Bullseye/configs/userconfig.cfg"
}
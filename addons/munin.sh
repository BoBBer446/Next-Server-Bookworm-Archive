...
cp /root/NeXt-Server-Bookworm/addons/vhosts/_munin.conf /etc/nginx/_munin.conf
...
echo "--------------------------------------------" >> /root/NeXt-Server-Bookworm/munin_login_data.txt
echo "Warning: It can take several minutes (or a restart), until the URL is working! (403 Error)" >> /root/NeXt-Server-Bookworm/munin_login_data.txt
echo "Munin Address: ${MYDOMAIN}/${MUNIN_PATH_NAME}/" >> /root/NeXt-Server-Bookworm/munin_login_data.txt
echo "MUNIN_HTTPAUTH_USER = ${MUNIN_HTTPAUTH_USER}" >> /root/NeXt-Server-Bookworm/munin_login_data.txt
echo "MUNIN_HTTPAUTH_PASS = ${MUNIN_HTTPAUTH_PASS}" >> /root/NeXt-Server-Bookworm/munin_login_data.txt

...
touch /root/NeXt-Server-Bullseye/teamspeak3_login_data.txt
touch /usr/local/ts3user/ts3server/.ts3server_license_accepted
timeout 10 sudo -u  ts3user /usr/local/ts3user/ts3server/ts3server_minimal_runscript.sh > /root/NeXt-Server-Bullseye/teamspeak3_login_data.txt

cp /root/NeXt-Server-Bullseye/addons/configs/ts3server /etc/init.d/ts3server
...
cat /root/NeXt-Server-Bullseye/addons/configs/ts3_ports.conf >> /root/NeXt-Server-Bullseye/configs/blocked_ports.conf

systemctl -q restart arno-iptables-firewall.service

echo "--------------------------------------------" >> /root/NeXt-Server-Bullseye/teamspeak3_login_data.txt
echo "Teamspeak 3" >> /root/NeXt-Server-Bullseye/teamspeak3_login_data.txt
echo "--------------------------------------------" >> /root/NeXt-Server-Bullseye/teamspeak3_login_data.txt
echo "TS3 Server Login = Look at: ts3serverdata.txt in the NeXt-Server Folder" >> /root/NeXt-Server-Bullseye/teamspeak3_login_data.txt
echo "TS3 Server commands = /etc/init.d/ts3server start and /etc/init.d/ts3server stop" >> /root/NeXt-Server-Bullseye/teamspeak3_login_data.txt

sed_replace_word "TS3_IS_INSTALLED=\"0"\" "TS3_IS_INSTALLED=\"1"\" "/root/NeXt-Server-Bullseye/configs/userconfig.cfg"

dialog_msg "Please save the shown login information on next page"
cat /root/NeXt-Server-Bullseye/teamspeak3_login_data.txt
continue_or_exit
}
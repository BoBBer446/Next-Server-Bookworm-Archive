...
if [[ ${CON_DL_FAILED} = "1" ]]; then
	dpkg -i /root/NeXt-Server-Bookworm/includes/python3-mysql.connector_${MYSQL_CONNECTOR_LOCAL}_all.deb >>"${main_log}" 2>>"${err_log}" || error_exit
else
...
echo "MAILSERVER_MANAGEVMAIL_PASS: $MAILSERVER_MANAGEVMAIL_PASS" >> /root/NeXt-Server-Bookworm/login_information.txt
echo "#------------------------------------------------------------------------------#" >> /root/NeXt-Server-Bookworm/login_information.txt
echo "" >> /root/NeXt-Server-Bookworm/login_information.txt
}

...
if [[ $INPUT_NEW_SSH_PORT =~ ^-?[0-9]+$ ]]; then
    if [ ${#INPUT_NEW_SSH_PORT} -ge 4 ]; then
       ...
       dialog --clear
        ...
        echo "#Date of change $(date +"%d-%m-%Y_%H_%M_%S")" >> /root/NeXt-Server-Bullseye/login_information.txt
        echo "#NEW_SSH_PORT: $NEW_SSH_PORT" >> /root/NeXt-Server-Bullseye/login_information.txt
        echo "#------------------------------------------------------------------------------#" >> /root/NeXt-Server-Bullseye/login_information.txt
        echo "" >> /root/NeXt-Server-Bullseye/login_information.txt
...
echo
echo
echo "You can find your New SSH key at /root/NeXt-Server-Bullseye/ssh_privatekey.txt"
echo
echo
...
cat /root/NeXt-Server-Bullseye/ssh_privatekey.txt
continue_or_exit
menu_options_openssh
;;
